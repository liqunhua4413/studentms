...
export default {
  methods: {
    deleteDepartment(row) {
      const that = this
      axios.get('/department/deleteById/' + row.id).then(function (resp) {
        if (resp.data === true) {
          that.$message({
            showClose: true,
            message: '删除成功',
          ...
          if (that.tmpList === null) {
            ...
            that.$router.push('/queryDepartment')
          }
        } else {
          that.$message({
            showClose: true,
  ...
  created() {
    ...
    if (this.$route.query.ruleForm === undefined || (this.ruleForm && this.ruleForm.name === null)) {
      ...
      axios.post('/department/findBySearch', this.ruleForm).then(function (resp) {
