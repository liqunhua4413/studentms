<template>
  <div>
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="name" label="年级名称" width="180"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteRow(row) {
      const that = this;
      this.axios.get('/gradeLevel/deleteById/' + row.id).then((resp) => {
        if (resp.data === true) {
    ...
    editor(row) {
      this.$router.push({ path: '/editorGradeLevel', query: { id: row.id } });
    }
  }
};
</script>
