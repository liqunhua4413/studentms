...
export default {
  methods: {
    ...
    changePage(page) {
      ...
      if (this.tmpList === null) {
        const that = this
        axios.get('/student/findByPage/' + page + '/' + that.pageSize).then(function (resp) {
  ...
  created() {
    ...
    if (this.$route.query.ruleForm === undefined || (this.ruleForm.sid === null && this.ruleForm.sname === null)) {
      ...
      axios.get('/student/findByPage/0/' + that.pageSize).then(function (resp) {
        that.tableData = resp.data
      })
    }
    else {
      ...
      axios.post('/student/findBySearch', this.ruleForm).then(function (resp) {
        console.log('获取查询数据：')
