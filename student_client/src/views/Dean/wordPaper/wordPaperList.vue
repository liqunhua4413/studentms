...
export default {
  data() {
    return {
      ...
      uploadBy: sessionStorage.getItem('name') || '',
      ruleForm: { fileName: null, uploadBy: null },
      tableData: [],
      rules: {}
    }
  },
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.axios.post('/paper/findBySearch', this.ruleForm).then(resp => {
    ...
    deletePaper(row) {
      this.axios.get('/paper/deleteById/' + row.id).then(resp => {
        if (resp.data === true) {
    ...
    loadData() {
      this.axios.get('/paper/findAll').then(resp => { this.tableData = resp.data })
    }
  },
  created() {
    this.loadData()
