    ...
    <el-card>
      ...
      <el-form :model="form" :inline="true" label-width="100px" class="query-form">
        ...
        <el-form-item label="课程">
          <el-select v-model="form.courseId" placeholder="请选择课程" clearable filterable style="width: 180px;">
...
export default {
  ...
  methods: {
    ...
    loadCourses () {
      this.axios.get('/dean/courses').then(resp => {
    ...
    buildQueryParams () {
      ...
      if (this.form.studentCollegeId) p.studentCollegeId = this.form.studentCollegeId
      if (this.form.studentMajorId) p.studentMajorId = this.form.studentMajorId
      if (this.form.studentClassId) p.studentClassId = this.form.studentClassId
    ...
    query () {
      ...
      this.axios.post('/dean/grade/query', params).then(resp => {
        this.list = resp.data || []
        this.page = 1
